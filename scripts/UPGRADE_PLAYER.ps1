# This script upgrades the video player to the enhanced version with playlist and custom controls
...
$newCSS = @"
    <style>
/* ========================================
   VIDEO PLAYER WINDOW STYLES (ENHANCED)
   ======================================== */
...
.video-player-window {
    ...
    z-index: 10001;
    width: 700px; /* Wider for playlist */
...
# 3. DEFINE NEW JS
$newJS = @"
        // Video Player Functionality (Enhanced)
        const videoFiles = [
        ...
        const playPauseBtnVideo = document.getElementById('play-pause-btn-video');
        ...
        function initPlaylist() {
            ...
            videoFiles.forEach((file, index) => {
                ...
                let displayName = file.split('/').pop().replace('.mp4', '');
        ...
        function toggleVideoPlayer() {
            ...
            if (videoWindow.style.display === 'none') {
                videoWindow.style.display = 'flex'; // Changed to flex for layout
                initPlaylist();
        ...
        function loadVideo(index) {
            if (index >= 0 && index < videoFiles.length) {
                ...
                videoSource.src = 'images/' + encodeURI(videoFiles[index]);
                videoElement.load();
...
# 4. REPLACE CONTENT
# Replace CSS
$content = $content -replace '(?s)<style>.*?VIDEO PLAYER WINDOW STYLES.*?</style>', $newCSS

# Replace HTML
$content = $content -replace '(?s)<div id="video-player-window".*?</div>\s*</div>', $newHTML

# Replace JS
# We need to be careful with regex for JS replacement.
# We'll look for the start of the video player JS and replace until the end of the script block (or next script block)
# Assuming it's the last block of code in the main script tag
$content = $content -replace '(?s)// Video Player Functionality.*?videoPlayerHeader\.style\.cursor = ''grab'';\s*?}', $newJS

# Write back to file
Set-Content -Path $path -Value $content -NoNewline

Write-Host "Upgraded video player to enhanced version!" -ForegroundColor Green
