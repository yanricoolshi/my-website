# FINAL DEFINITIVE FIX - Enhanced Video Player
# This script carefully adds the enhanced video player without breaking anything

$path = "C:\Users\akoda\Documents\GitHub\my-website\index.html"
$content = Get-Content -Path $path -Raw

# 1. ADD CSS (Insert after style.css link, before </head>)
$cssBlock = @"
    <style>
...
$content = $content -replace '(</head>)', "$cssBlock`r`n`$1"

# 2. ADD HTML (Insert AFTER the comment <!-- Video Player Window -->)
$htmlBlock = @"
...
# Replace the old video player HTML
# Find the old marker and replace it
$oldVideoPlayerPattern = '(?s)    <!-- Video Player Window -->.*?</div>\s*</div>'
$content = $content -replace $oldVideoPlayerPattern, $htmlBlock

# 3. ADD JAVASCRIPT (Replace the old video player JS)
$jsBlock = @"
        ...
        function updatePlayPauseIcon() {
            if (!playPauseBtnVideo || !videoElement) return;
            if (videoElement.paused) {
...
# Find and replace the old video player JS
$oldJSPattern = '(?s)        // Video Player Functionality.*?videoPlayerHeader\.style\.cursor = .grab.;\s*}'
$content = $content -replace $oldJSPattern, $jsBlock

# Write the file
Set-Content -Path $path -Value $content -NoNewline

Write-Host "Enhanced video player installed successfully!" -ForegroundColor Green
